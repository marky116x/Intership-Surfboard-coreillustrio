...
export class MovieService {
  ...
  getPsy() {
    return this.http.get(
      `${this.apiUrl}/discover/tv?api_key=${this.apiKey}&with_companies=420`
    );
  }
  ...
  getPicks(): Observable<any> {
    return this.http.get(
      `${this.apiUrl}/discover/tv?api_key=${this.apiKey}&with_keywords=180547&sort_by=release_date`
    );
  }

  getOrignals(): Observable<any> {
    return this.http.get(
      `${this.apiUrl}/discover/tv?api_key=${this.apiKey}&with_watch_providers=337&watch_region=US`
    );
  }
